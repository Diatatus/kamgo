...
class user{
    ...
    function login($user_name,$password,$conn)
    {
        ...
        if(sizeof($errors)==0 )
        {
            $password = md5($password);
             ...
             if ($result->num_rows > 0) {
                 ...
                 while($row = $result->fetch_assoc()){
                     ...
                     if($row['isblock']==0)
                     {
                        echo '<p class="bg-danger text-center">Vous n êtes pas approuvé par ADMIN</p>';
                     }
                     if($row['isblock']==1 && $row['is_admin']==0){
                        ...
                        if(isset($_SESSION['book']))
                        {
                            ...
                            echo '<script>alert("Votre demande de trajet de '.$pickup.' à '.$drop.' PAR '.$cabtype.' a été envoyé");
             ...
             else 
            {
                ...
                echo '<p class="bg-danger text-center">Informations de connexion invalide</p>';
    ...
    function register($username,$password,$password2,$email,$mobile,$datetime,$conn)
     {
        ...
        if ($password != $password2) {
            ...
            echo '<p class="bg-danger text-center">le mot de passe doit être le même</p>';
        }
        ...
        if ($password == $password2) {
            ...
            if ($result->num_rows > 0) {
                ...
                echo '<p class="bg-danger text-center">Identifiant email déjà enregistré</p>';
        ...
        if(count($errors)==0 )
        {
            ...
            if ($conn->query($sql) === true) {
                echo '<script>alert("Inscription réussie, accès à la page de connexion");
    ...
    function uprof($name,$mobile,$ida,$conn)
    {
        $sql = "UPDATE user SET name='".$name."', mobile='".$mobile."' WHERE user_id=$ida";
            ...
            if ($conn->query($sql) === true) {
                ...
                echo '<script>alert("Mise à jour du profil réussie");
                window.location.href = "usrprofile.php"</script>';
                
               // echo '<p class="bg-success text-center">Profile Edited Successful</p>';
            }
             else {
               echo '<p class="bg-danger text-center">Quelque chose s est mal passé</p>';
    ...
    function changep($old,$new,$rnew,$idp,$conn)
    {
        ...
        if($pass==$old)
        {
            ...
            if ($new != $rnew) {
                $errors[] = array('input'=>'password','msg'=>'password should be same');
                echo '<p class="bg-danger text-center">Le nouveau mot de passe et le nouveau mot de passe doivent être identiques</p>';
            }
            ...
            if(count($errors)==0 )
            {
                ...
                if ($conn->query($sql) === true) {
                    echo '<script>alert("Le mot de passe a été modifié avec succès, veuillez vous reconnecter");
            ...
            echo '<p class="bg-danger text-center">L ancien mot de passe ne correspond pas</p>';
        }
    }
