    ...
    function alocation($location,$distance,$available,$conn)
    {
        $errors=array();
            ...
            if ($result->num_rows > 0) {
                $errors[] = array('input' => 'result', 'msg' => 'Ce nom utilisateur existe déjà');
                echo '<p class="bg-danger text-center">Cet emplacement existe déjà</p>';
            }
        if(count($errors)==0 )
        {   
            ...
            if ($conn->query($sql) === true) {
                echo '<p class="bg-success text-center">Emplacement ajouté avec succès</p>';
            }
             else {
